...
const FOOTER = ({ isEdit, updateAt, pw, mode, share }) => `
    <div class="footer">
        ${isEdit ? `
            <div class="opt">
                <button class="opt-button opt-pw">${pw ? '修改' : '设置'}密码</button>
                ${SWITCHER('Markdown', mode === 'md', 'opt-mode')}
                ${SWITCHER('分享', share, 'opt-share')}
            </div>
            ` : ''
    }
        ...
        ${updateAt ? `<span class="last-modified">上次保存 ${dayjs.unix(updateAt).fromNow()}</span>` : ''}
    </div>
`
const MODAL = `
<div class="modal share-modal">
    ...
    <div class="modal-content">
        ...
        <div class="modal-body">
            ...
            <button class="opt-button">复制</button>
        </div>
    </div>
</div>
`
            ...
            <div class="layer_1">
                <div class="layer_2">
                    <div class="layer_3">
                        ...
                        <textarea id="contents" class="contents ${isEdit ? '' : 'hide'}" spellcheck="true" placeholder="看来你是第一个到这儿的人，写点什么吧...">${content}</textarea>
...
export const NeedPasswd = data => HTML({ tips: '这是一篇加密笔记，你必须先输入密码', showPwPrompt: true, ...data })
export const Page404 = data => HTML({ tips: '404，你要找的东西并不存在', ...data })